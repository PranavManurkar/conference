...
class RegistrationAdmin(admin.ModelAdmin):
    ...
    def mark_accepted(self, request, queryset):
        if not request.user.is_superuser:
            messages.error(request, "Only the main administrator can approve or reject registrations.")
            return
        ...
        self.message_user(request, f"{updated_count} registration(s) marked as Accepted. Syncing to sheet in background.")
    ...
    def mark_rejected(self, request, queryset):
        ...
        self.message_user(request, f"{updated_count} registration(s) marked as Rejected. Syncing to sheet in background.")
    ...
    def mark_under_process(self, request, queryset):
        ...
        self.message_user(request, f"{updated_count} registration(s) marked as Under Process. Syncing to sheet in background.")
...
class WorkshopRegistrationAdmin(admin.ModelAdmin):
    list_display = (
        "registration_reference", "full_name", "email",
        "participant_type", "fee_amount", "status",
